...
- Use a pre-saved hook to automatically generate a URL-friendly slug from the title
- Only regenerate the slug if the title changes.
- In the same pre-save hook, validate and normalaize the `date` to ISO format and ensure `time` is stored in a consistent format.
- Validate that required fields are present and non-empty.
- Add aunique index to slug
- Enable automatic timestamps.
- Use strict TypeScript types (no `any`)
- Write consise comments explaining key logic such as slug generation, date formatting and validation

2. `database/booking.model.ts`

Creae a strongly typed Mongoose schema and model called Booking with the following fields:

- `eventId` - ObjectId (reference to `Event`), required
- `email` - string, required, must be a valid email
- `ceratedAt` - date auto-generated
- `updatedAt` - date, auto-generated 
...
-Export both `Event` and Booking` models so they can be imported anywhere int he application from a single file. 
...
- Code should be production-graded, clean, type-safe and clear to understand.
...
You are a backend developer working on Next.js application with Mongoose and TypeScript. 

Your task is to;

- Create a new  file `lib/mongodb.ts` in the lib folder of a Next.js application.
- Set up Mongoose databsae connection to MongoDB using TypeScript with proper types (avoid using any).
